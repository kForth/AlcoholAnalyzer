...
def print_stats_to_html(data, filename):
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            "<head><script src='http://www.kryogenix.org/code/browser/sorttable/sorttable.js'></script></head>\n<body>\n<b>" + date + "</b>\n<table class='sortable'>\n")
        f.write(
            "<tr style='font-weight:bold'><td>Name</td><td>Code</td><td>Source</td><td>Type</td><td>Price</td><td>Quantity</td><td>Single Vol</td><td>Total Vol</td><td>Price Per Vol</td><td>Alc Vol</td><td>Price Per Alc</td></tr>")
        for d in data:
            f.write("<tr>")
            for e in d:
                if e == d[1]:
